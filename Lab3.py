...
def main():
    ...
    export_order_to_excl(order_id, order_df, orders_dir)
    return
...
# Group sales data by order ID and save to Excel sheets
  for order_id, order_df in sales_df.groupby('ORDER ID'):
   ...
   # Add the ground total row
   grand_total = order_df['TOTAL PRIC3E'].sum()
   grand_total_df = pd.DataFrame({'ITEAM PRICE' :  ['GRAND TOTAL:'], 'TOTAL PRICE':[grand_total]})
   order_df = pd.concat([order_df, grand_total_df])
   
def export_order_to_excl(order_id, order_df, orders_dir):
  ...
  os.path.join(orders_dir, order_file)
  
  sheet_name = f'Order # {order_id}'
  order_df.to_excel(order_path)
